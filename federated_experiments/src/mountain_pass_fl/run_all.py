...
def write_report(
    ...
    *,
    ...
    lines = []
    lines.append("# Experiment report: FL beacons for mountain passes\n")
    lines.append("## Segmented data\n")
    lines.append(f"- Clients/routes detected: **{segments['client_id'].nunique()}**")
    lines.append(f"- Segments generated: **{len(segments)}**")
    lines.append(f"- Total segmented distance: **{segments['segment_length_m'].sum()/1000:.2f} km**")
    lines.append("\n## Split by clients\n")
    for key, vals in split_info.items():
        lines.append(f"- {key}: {len(vals)} clients")
    ...
    lines.append("\n## Quick read\n")
    lines.append(
        "The main comparison is entry-to-exit: each method estimates the total energy required before entering the segment; "
        "the actual energy is computed on exit by summing `energy_used_cum_wh - energy_regen_cum_wh`. "
        "The B1 baseline assumes flat road and does not know the topography. The M1/M2 models learn a residual on top of that baseline using topographic features."
    )
    (out_dir / "REPORT.md").write_text("\n".join(lines), encoding="utf-8")


def main() -> None:
    parser = argparse.ArgumentParser(description="Run EV range + Flower FL experiments on CARLA CSV files")
    ...
    parser.add_argument("--rebuild-segments", action="store_true", help="Rebuild outputs/segments.csv even if it exists")
    parser.add_argument("--skip-flower", action="store_true", help="Skip the Flower experiments")
    parser.add_argument("--only-baselines", action="store_true", help="Only run segmentation and baselines")
    parser.add_argument("--quick", action="store_true", help="Use configs/quick.yaml if it exists")
    ...
    if args.rebuild_segments or not segment_path.exists():
        ...
        print("[1/7] Reading existing segments...")
        segments = pd.read_csv(segment_path)

    print(f"  segments={len(segments)} clients={segments['client_id'].nunique()}")
    print("[2/7] Running entry-to-exit baselines...")
    ...
    print("[3/7] Training centralized residual MLP model...")
    try:
        ...
        print(f"  ERROR centralized: {exc}")

    print("[4/7] Running Flower FedAvg/FedProx...")
    if args.skip_flower:
        skipped.append("Flower skipped due to --skip-flower")
    else:
        try:
    ...
    print("[5/7] Running prequential beacon evaluation...")
    try:
    ...
    print("[6/7] Aggregating metrics and risk analysis...")
