...
def run_flower(
    ...
    *,
    ...
    try:
        ...
        raise ImportError(
            "Flower is not installed. Run: pip install -r requirements.txt "
            "or use --skip-flower to skip this experiment."
        ) from exc
    ...
    if len(train_df) == 0:
        raise RuntimeError("No training segments for Flower")
    if len(val_df) == 0:
        val_df = train_df
    if len(test_df) == 0:
        test_df = val_df
    ...
    if strategy_name.lower() == "fedprox":
        ...
        if FedProx is None:
            raise RuntimeError("The installed Flower version does not expose fl.server.strategy.FedProx")
