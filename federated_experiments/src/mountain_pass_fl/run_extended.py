...
def _build_or_load_segments(args, cfg: dict[str, Any], out_dir: Path) -> pd.DataFrame:
    ...
    if args.rebuild_segments or not segment_path.exists():
        ...
        if min_route_distance_m:
            ...
            print(f"  filter min_route_distance_m={min_route_distance_m}: clients {before} -> {after}")
        segments.to_csv(segment_path, index=False)
    else:
        print("[1/6] Reading existing segments...")
        segments = pd.read_csv(segment_path)
    meta = client_metadata(segments)
    meta.to_csv(out_dir / "client_metadata.csv", index=False)
    print(f"  segments={len(segments)} clients={segments['client_id'].nunique()} total_distance={segments['segment_length_m'].sum()/1000:.2f} km")
    return segments
...
def write_extended_report(
    ...
    *,
    ...
    lines: list[str] = []
    lines.append("# Extended report: cross-validation and beacon learning curve\n")
    lines.append("## Data\n")
    lines.append(f"- Clients/routes: **{segments['client_id'].nunique()}**")
    lines.append(f"- Segments: **{len(segments)}**")
    lines.append(f"- Total segmented distance: **{segments['segment_length_m'].sum()/1000:.2f} km**")
    lines.append("\n## What this report measures\n")
    lines.append(
        "The main metric remains entry-to-exit: energy estimated before entering the mountain pass versus actual energy on exit. "
        "Cross-validation is done over complete clients/routes, never over rows or random segments."
    )
    if cv_summary is not None and len(cv_summary):
        lines.append("\n## Cross-validation: aggregated summary by protocol\n")
    ...
    if cv_risk_summary is not None and len(cv_risk_summary):
        lines.append("\n## Cross-validation: risk by protocol\n")
    ...
    if lc_summary is not None and len(lc_summary):
        lines.append("\n## Beacon learning curve\n")
        lines.append(
            "`remaining_unseen` evaluates the model, after seeing k cars, on the cars that have not passed yet. "
            "`all_routes_replay` is diagnostic: after seeing k cars, it re-evaluates all routes, so it should not be read as pure generalization when k > 0."
        )
    ...
    if lc_risk_summary is not None and len(lc_risk_summary):
        lines.append("\n## Learning curve: false 'you can cross' calls\n")
    ...
    lines.append(
        "Look especially for two effects: (1) that methods without topography keep a negative bias, i.e. they underestimate energy; "
        "and (2) that the beacon curve reduces MAE and false-safe rate as `n_seen_clients` increases."
    )
    (out_dir / "EXTENDED_REPORT.md").write_text("\n".join(lines), encoding="utf-8")


def main() -> None:
    parser = argparse.ArgumentParser(description="Extended experiments: CV + beacon learning curve")
    ...
    parser.add_argument("--skip-flower", action="store_true", help="Skip the Flower checkpoints and any CV Flower runs")
    parser.add_argument("--skip-cv", action="store_true")
    parser.add_argument("--skip-learning-curve", action="store_true")
    parser.add_argument("--quick", action="store_true", help="Use configs/quick_extended.yaml if it exists")
    ...
    if not args.skip_cv and bool(_get(cfg, "cross_validation.enabled", True)):
        print("[2/6] Running cross-validation over routes/clients...")
    ...
    if not args.skip_learning_curve and bool(_get(cfg, "learning_curve.enabled", True)):
        ...
        if not args.skip_flower:
            print("[4/6] Running explicit Flower checkpoints for the curve...")
            ...
            print("[4/6] Flower checkpoints skipped due to --skip-flower")
    else:
        print("[3/6] Learning curve skipped")
        print("[4/6] Flower checkpoints skipped")
    ...
    print("[6/6] Writing extended report...")
    ...
    print(f"Done. Extended results in: {out_dir.resolve()}")
