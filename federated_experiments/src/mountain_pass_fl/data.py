...
def compute_cumulative_distance(df: pd.DataFrame) -> pd.Series:
    ...
    if not all(c in df.columns for c in required):
        raise ValueError("The CSV must contain pos_x, pos_y and pos_z to compute cumulative distance")
...
def prepare_raw_dataframe(path: str | Path) -> tuple[pd.DataFrame, dict[str, object]]:
    ...
    if "energy_used_cum_wh" not in df.columns or "energy_regen_cum_wh" not in df.columns:
        raise ValueError(f"{path.name} does not contain energy_used_cum_wh and energy_regen_cum_wh")
    ...
    for path in files:
        try:
            ...
            if len(seg):
                ...
                errors.append({"file": path.name, "error": "no valid segments"})
        except Exception as exc:
            errors.append({"file": path.name, "error": str(exc)})
    if errors:
        print("Warnings during segmentation:")
        for e in errors:
            print(f"  - {e['file']}: {e['error']}")
    if not tables:
        raise RuntimeError("No valid segment was generated")
