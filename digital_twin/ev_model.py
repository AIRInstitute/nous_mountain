...
ev_model.py
============
Modular energy model with a library of parameterized vehicles.
"""
...
def get_vehicle_params(vehicle_key):
    """Return a copy of the parameter dictionary for the requested vehicle."""
    if vehicle_key not in EV_LIBRARY:
        options = ', '.join(EV_LIBRARY.keys())
        raise KeyError(
            f"Vehicle '{vehicle_key}' not found. Options: {options}"
        )
...
def hvac_aux_power(t_amb_c):
    """HVAC auxiliary power (W) as a function of ambient temperature."""
    base = 300.0
...
def battery_capacity_factor(t_amb_c):
    """Multiplicative factor (0..1) applied to usable capacity vs ambient temperature."""
    if t_amb_c >= 20.0:
        return 1.0
    if t_amb_c >= 0.0:
...
def rolling_coefficient(c_rr_base, road_condition):
    """Adjust C_rr according to the pavement condition."""
    multipliers = {
...
class ElectricVehicleModel:
    """Longitudinal EV model with optional weather coupling."""
