...
This folder holds a factorial experimental dataset produced by simulating a digital twin of an electric vehicle (EV) traversing a representative stretch of the **Puerto de la Quesera** (Sistema Central, GU-186 road). The dataset comprises 12 scenarios resulting from the factorial combination of vehicle, initial state of charge (SOC), and weather condition. The code that generated it lives in [`../digital_twin/`](../digital_twin/); the federated-learning experiments that consume it live in [`../federated_experiments/`](../federated_experiments/). See the [repository root README](../README.md) for the overall layout.
...
The parameters are stored in `../digital_twin/ev_model.py` under the `EV_LIBRARY` dictionary. Sources: manufacturer specifications, EPA test data, ev-database.org, ADAC tests.
...
The code that produced these files (`ev_model.py` and `carla_acquisition.py`) is in [`../digital_twin/`](../digital_twin/).
...
df = pd.read_csv('data/datos_grid_S03_tesla_SOC90_snow_neg5.csv')
...
Developed by Emmanuel Cuevas, Yeray Mezquita Martín, Albano Carrera González, and Diego Valdeolmillos Villaverde. For questions about the data, model assumptions, or experimental design, please open an issue in this repository.
